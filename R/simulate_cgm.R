simulate_cgm_panel <- function(n_patients = 50,
  ...
  sim_list <- lapply(patient_ids, function(id) {
    ...
    tt <- start_time + (t_idx - 1) * interval_mins * 60
    ...
    p_missing_start <- plogis(-5 + 0.015 * lag_glucose + 0.6 * (lubridate::hour(tt) %in% c(0,1,2,3)))
    missing_start <- rbinom(n_time, 1, p_missing_start)
    
    duration_steps <- ifelse(
      missing_start == 1,
      pmax(1, round(exp(rnorm(n_time, log(6), 0.4)))),
      0
    )
